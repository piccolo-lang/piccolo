...
\kw{begin} \\
\pindent  \kw{foreach} $chan$ : \myref{Channel} in \myref{KnowsSetKnows}($pt.knows$) \\
...
\begin{program}
  ...
  \synchro{\myref{ProcessYield}($pt$:\myref{PiThread})} \\
  \kw{return}
\end{program}
...
\begin{program}
  \compiletime{Compile(\kw{new}($c$)) $\rightarrow$} \\ \\
  \pindent\kw{var} $channel$ : \myref{Channel} \\
  \>$channel \leftarrow$ \myref{GenerateChannel}($pt$) \\
...
\begin{program}
  ...
  \>\myref{KnowsSetSwitch}($pt.knows, newchan$, KNOWN) \\
  \> \kw{return} $newchan$ \\
  \kw{endfunction}
\end{program}
...
\begin{program}
  \compiletime{Compile(\kw{spawn}\{D($v_1$:T$_1, ..., v_n$:T$_n$)\}) $\rightarrow$} \\ \\
  \kw{begin} \\
  \pindent\kw{var} $args$ : Array[$n$ * Value] \\
...
\section{Compilation of guarded choice}
