...
\begin{program}
  ...
  \kw{begin} \\
  \pindent\compiletime{\kw{cvar} $after\_wait\_fuel$ : \kw{label}} \\
  \>\kw{var} $tryresult$ : \{ENABLED, DISABLED, COMMIT\} \\
...
\begin{program}
  ...
  \kw{begin} \\
  \pindent\compiletime{\kw{cvar} $label\_endoftry$ : \kw{label}} \\
  \>\compiletime{$label\_endoftry \leftarrow$ GenerateLabel()} \\
  \>\kw{if} SetAdd($chans, pt.env[c]$) \kw{then} \\
  \>\pindent\synchro{Acquire($pt.env[c].lock$)} \\
  \>\kw{endif} \\
  \>\kw{if} $c.globalrc = 1$ \kw{then} \\
  \>\pindent$tryresult \leftarrow$ DISABLED \\
  \>\>\kw{goto} $@label\_endoftry$ \\
  \>\kw{endif} \\
...
\begin{program}
  \compiletime{CompileTryAction($c?x$:T, Set[\myref{Channel}]) $\rightarrow$} \\ \\
  \kw{begin} \\
  \pindent\compiletime{\kw{cvar} $label\_endoftry$ : \kw{label}} \\
  \>\compiletime{$label\_endoftry \leftarrow$ GenerateLabel()} \\
  \>\kw{if} SetAdd($chans, pt.env[c]$) \kw{then} \\
  ...
  \> \\
  \>\kw{do} \\
  \>\pindent$commit \leftarrow$ \myref{FetchOutputCommitment}($pt.env[c]$) \\
  \>\>\kw{if} $commit =$ \kw{null} \kw{then} \\
  \>\>\pindent$tryresult \leftarrow$ COMMIT \\
  \>\>\>\kw{goto} $@label\_endoftry$ \\
  \>\>\kw{endif} \\
  ...
  \>\>\\
  \>\>\kw{if} $ok =$ VALID\_COMMITMENT \kw{then} \\
  \>\>\pindent\kw{var} $val$ : Value \\
  \>\>\>$val \leftarrow$ commit.evalfunc($commit.thread$) \\
  \>\>\>$pt.env[x] \leftarrow val$ \\
