...
\begin{program}
  ...
  \kw{begin} \\
   ...
   \> \kw{var} $outchan$ : \myref{Channel} \\
   \> $outchan \leftarrow pt.env[c]$ \\
   \> $ok \leftarrow $ ProcessAcquireChannel($pt$, $outchan$) \\
  ...
  \>\pindent \synchro{ReleaseChannel($outchan$)} \\
  \>\>\compiletime{\myref{CompileExpr}($e$)} \\
  \>\>$commit.thread.env[commit.refval] \leftarrow pt.val$ \\
  \>\>\compiletime{\kw{cif} ischannel(T) \kw{then}} \\
  \>\>\pindent\kw{if} \myref{KnowsRegister}($commit.thread.knows, commit.thread.env[commit.refval]$) \kw{then} \\
  \>\>\>\pindent \myref{ChannelRef}($commit.thread.env[commit.refval]$) \\
  \>\>\>\kw{endif} \\
  \>\>\compiletime{\kw{cendif}} \\
...
\algosection{Compilation outline for prefix primitives}
\begin{program}
\compiletime{CompilePrefix(\code{\#}$prim(e_1$:T$_1, ..., e_n$:T$_n)$) $\rightarrow$} \\
  \kw{begin} \\
  \pindent \compiletime{\kw{cfor} $i$ : \kw{int} \kw{from} $1$ \kw{to} $n$ \kw{do}} \\
  \> \pindent \kw{var} $v_i$ : T$_i$ \\
  \> \> \compiletime{CompileExpr($e_i$)} \\
