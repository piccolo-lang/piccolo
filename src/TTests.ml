...
let test_call5 = "def Call5(x:int) = Call5(#core/arith:add(3,3))";; (* ok *)
...
let test_tuple2 = "def Out(c:chan<int>) = c!(42, (true, false), 44), end";; (* tuple in tuple type error *)

let test_tuple3 = "def Out(c:chan<(int * int)>) = c!(42, #core/arith:add(3, 4)), end";; (* tuple in tuple *)
